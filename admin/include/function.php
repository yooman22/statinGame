<?php
function AdminNavActivate( $nav_item ){
  $position = strpos( $_SERVER['REQUEST_URI'],$nav_item->url );
  if( $position === false ){
    return false;
  }else{
    return true;
  }

}

function addHTMLTagLine( $tag ){
  echo $tag."\n";
}

function getNavUrl( $slug ){
  global $nav_arr;
  foreach( $nav_arr as $nav_item ){
    if( $nav_item->slug == $slug ){
      return $nav_item->url;
    }
  }
  return $slug;
}

function printJson( $message = null, $arr = null ){
  $result_arr = array();
  $result_arr[] = '"message":"'.$message.'"';
  if( $arr ) {
    foreach( $arr as $key => $value ){
      if( is_string($value) ){
        $result_arr[] = '"'.$key.'":"'.$value.'"';
      }else if( is_array($value)){
        $result_arr[] = '"'.$key.'":['.implode($value,',').']';
      }else{
        $result_arr[] = '"'.$key.'":'.$value.'';
      }
    }
  }
  return '{'.implode($result_arr,',').'}';
}

function printSelected( $key, $value ){
  if( $key == $value ){
    echo " selected";
  }
}

function getUserLevelName( $user_level ){
  switch( $user_level ){
    case 0:
      return '이용자';
    case 9:
      return '운영자';
    case 10:
      return '최고관리자';
  }
}

function getBrowser($u_agent)
{
    //$u_agent = $_SERVER['HTTP_USER_AGENT'];
    $bname = 'Unknown';
    $platform = 'Unknown';
    $version= "";

    //First get the platform?
    if (preg_match('/linux/i', $u_agent)) {
        $platform = 'linux';
    }
    elseif (preg_match('/macintosh|mac os x/i', $u_agent)) {
        $platform = 'mac';
    }
    elseif (preg_match('/windows|win32/i', $u_agent)) {
        $platform = 'windows';
    }

    // Next get the name of the useragent yes seperately and for good reason
    if(preg_match('/MSIE/i',$u_agent) && !preg_match('/Opera/i',$u_agent))
    {
        $bname = 'Internet Explorer';
        $ub = "MSIE";
    }
    elseif(preg_match('/Firefox/i',$u_agent))
    {
        $bname = 'Mozilla Firefox';
        $ub = "Firefox";
    }
    elseif(preg_match('/Chrome/i',$u_agent))
    {
        $bname = 'Google Chrome';
        $ub = "Chrome";
    }
    elseif(preg_match('/Safari/i',$u_agent))
    {
        $bname = 'Apple Safari';
        $ub = "Safari";
    }
    elseif(preg_match('/Opera/i',$u_agent))
    {
        $bname = 'Opera';
        $ub = "Opera";
    }
    elseif(preg_match('/Netscape/i',$u_agent))
    {
        $bname = 'Netscape';
        $ub = "Netscape";
    }

    // finally get the correct version number
    $known = array('Version', $ub, 'other');
    $pattern = '#(?<browser>' . join('|', $known) .
    ')[/ ]+(?<version>[0-9.|a-zA-Z.]*)#';
    if (!preg_match_all($pattern, $u_agent, $matches)) {
        // we have no matching number just continue
    }

    // see how many we have
    $i = count($matches['browser']);
    if ($i != 1) {
        //we will have two since we are not using 'other' argument yet
        //see if version is before or after the name
        if (strripos($u_agent,"Version") < strripos($u_agent,$ub)){
            $version= $matches['version'][0];
        }
        else {
            $version= $matches['version'][1];
        }
    }
    else {
        $version= $matches['version'][0];
    }

    // check if we have a number
    if ($version==null || $version=="") {$version="?";}

    return array(
        'userAgent' => $u_agent,
        'name'      => $bname,
        'version'   => $version,
        'platform'  => $platform,
        'pattern'    => $pattern
    );
}

function numberOnly( $c ){
  return preg_replace('/\D/', '', $c);
}

function getTextIdRandom( $str ){
  return $str."0".mt_rand(1,1000);
}

function getDateFromTime( $t ){
  if( $t == '' ){
    return date( 'Y-m-d' );
  }else{
    $arr = explode(' ',$t);
    return $arr[0];
  }
}

?>
